...
[dPay](https://github.com/dpays/dpay), a blockchain powered by Graphene
...
* Assisting with software development for dpays.io
...
```bash
docker run -it -p 8080:8080 dsites/frontend
```

Environment variables can be added like this:

```bash
docker run -it --env SDC_DATABASE_URL="mysql://user:pass@hostname/databasename" -p 8080:8080 dsites/frontend
```
...
```bash
git clone https://github.com/dsites/frontend
cd condenser
...
By default you will be connected to dPay.io's public dPay node at
`wss://d.dpays.io`. This is actually on the real blockchain and
...
[https://github.com/dpays/dpay](https://github.com/dpays/dpay).
...
```bash
export SDC_CLIENT_DPAYD_URL="wss://d.dpays.io"
export SDC_SERVER_DPAYD_URL="wss://d.dpays.io"
```
...
To run a Selenium test suite, start the condenser docker image with a name `condenser` (like `docker run --name condenser -itp 8080:8080 dsites/frontend:latest`) and then run the blackboxtest image attached to the condneser image's network:

```
docker build -t=dsites/frontend-blackboxtest blackboxtest/
docker run --network container:condenser dsites/frontend-blackboxtest:latest
...
If you find a security issue please report details to: labs@benchx.io or labs@dpays.io
