 ...
 *    - basic sanitization?
 *  2. dSite.io Rendering - add in proprietary dSite.io functions/links
...
function link(state, child) {
    ...
    if (url) {
        state.links.add(url);
        if (state.mutate) {
            // If this link is not relative, http, https, or dpay -- add https.
            if (!/^((#)|(\/(?!\/))|(((dpay|https?):)?\/\/))/.test(url)) {
            ...
            if (
                (url.indexOf('#') !== 0 && // Allow in-page links
                    (child.textContent.match(/(www\.)?dsite\.io/i) &&
                        !url.match(
                            /https?:\/\/(.*@)?(www\.)?dsite\.io/i
                        ))) ||
...
// For all img elements with non-local URLs, prepend the proxy URL (e.g. `https://img0.dsite.io/0x0/`)
...
function ipfsPrefix(url) {
    if ($STM_Config.ipfs_prefix) {
        // Convert //ipfs/xxx  or /ipfs/xxx  into  https://dsite.io/ipfs/xxxxx
        if (/^\/?\/ipfs\//.test(url)) {
