            ...
            >
                <div className="row">
                    <div className="small-12 columns">
                        <h1>
                            {tt('converttobex_jsx.convert_to_LIQUID_TOKEN', {
                                LIQUID_TOKEN,
                            })}
                        </h1>
                        <p>
                            {tt(
                                'converttobex_jsx.DEBT_TOKEN_will_be_unavailable',
                                { DEBT_TOKEN }
                            )}
                        </p>
                        <p>
                            {tt(
                                'converttobex_jsx.your_existing_DEBT_TOKEN_are_liquid_and_transferable',
                                { link: tt('g.buy_or_sell'), DEBT_TOKEN }
                            )}
                        </p>
                        <p>
                            {tt(
                                'converttobex_jsx.this_is_a_price_feed_conversion'
                            )}
                        </p>
                    </div>
                </div>
...
export default reduxForm(
    ...
    (state, ownProps) => {
        ...
        const bbd_balance = account.get('bbd_balance');
        const max = bbd_balance.split(' ')[0];
                    ...
                    successCallback: () => {
                        ...
                        dispatch(
                            appActions.addNotification({
                                key: 'convert_bd_to_bex_' + Date.now(),
